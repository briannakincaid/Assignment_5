...
\subsection{Tidy/Clean}

The goal here is to make the vegetable dataset tidy so that we can then more easily work with it. In order to make it tidy, we need to make it so each variable has its own column, each observation has its own row, and each value has its own cell. 

\subsubsection{Clean Columns}
...
\subsubsection{Separate Columns}
...
The only two that don't have two values are TOTAL and FERTILIZER. The rest are obviously broken up into Domain and SubDomain. Let's filter out the TOTAL and FERTILIZER rows for now. 
...
veg.2 now holds the rows that have TOTAL or FERTILIZER in the Domain column and NA in the SubDomain column. We'll bind this back with the whole data set later. \par
...
veg.1 now just holds the rows that don't have TOTAL or FERTILIZER in the Domain column and the Domain column has been separated into Domain and SubDomain.\par
...
Most of the rows are in the form DOMAIN, SUBDOMAIN: (Description). The only rows that are not in this form have the value "NOT SPECIFIED" in the Category column. Let's filter so we just have the rows with "NOT SPECIFIED" in the Category column.
...
veg.3 now just holds the rows that now have NA in the Description column. We'll bind these back onto the whole data set later. 
...
veg.1 now holds the rows that were of the form DOMAIN, SUBDOMAIN: (Description) in the Category column. We separted DOMAIN, SUBDOMAIN and (Description) and renamed the column with (Description) to "Description". Let's bind them back together
<<>>=
veg.1 <- rbind(veg.1, veg.3)
@

\paragraph{Name (Category, pt. 2)}

<<>>=
unique(veg.1[,"Description"])
@

We want to keep only the rows that have the form NAME = NUMBER and separate it into two separate description files.
...
veg.4 now holds all the rows that had the form NAME = NUMBER, now separated into two Description columns.\par
...
Before we go on, there are (Z) values in the Value column. \par
...
It does not work for Vegetable Totals. It seems that the data is too large for spread to handle, and there is some sort of memory problem. We could break it down into tibbles of similar size to BrusselsSprouts, but that may be too tedius. We can get the same result as spread() by the doing the following (which is still tedius, and I would still much rather use spread to do this, but again, it's not working...):

\subparagraph{DIY spread()}
...
\subsubsections{Adding RfD values from EPA Dashboard}

RfD is an estimate (with uncertainty spanning perhaps an order of magnitude) of a daily exposure to the human population (including sensitive subgroups) that is likely to be without an appreciable risk of deleterious effects during a lifetime.
